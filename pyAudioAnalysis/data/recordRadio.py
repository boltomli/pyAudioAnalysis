...
class StreamRecorder:
    ...
    def _build_pipeline(self):
        ...
        if recording_bin:
            self._pipeline.add(recording_bin)
        if listening_bin:
            self._pipeline.add(listening_bin)
        ...
        if recording_bin:
            splitter.link(recording_bin)
        if listening_bin:
            splitter.link(listening_bin)
    ...
    def _create_recording_bin(self):
        ...
        caps.set_property(
            "caps",
            Gst.Caps.from_string("audio/x-raw, rate={0}".format(self.samplerate)),
        )
        ...
        recording_bin.add_pad(
            Gst.GhostPad.new("recording_sink", queue.get_static_pad("sink"))
        )
    ...
    def _create_listening_bin(self):
        ...
        listening_bin.add_pad(
            Gst.GhostPad.new("listening_sink", queue.get_static_pad("sink"))
        )
