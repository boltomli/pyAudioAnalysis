...
def main(argv):
    if argv[1] == "-shortTerm":
        ...
        for i in range(nExp):
            ...
            perTime1 = duration / (t2 - t1)
            print(
                "Mid-term feature extraction + classification \t {0:.1f} x realtime".format(
                    perTime1
                )
            )
        ...
        for i in range(nExp):
            ...
            duration = x.shape[0] / float(Fs)
            t1 = time.time()
            [flagsInd, classesAll, acc] = aS.mid_term_file_classification(
                "diarizationExample.wav", "svmSM", "svm", False, ""
            )
            t2 = time.time()
            perTime1 = duration / (t2 - t1)
            print(
                "Fix-sized classification - segmentation \t {0:.1f} x realtime".format(
                    perTime1
                )
            )
        ...
        for i in range(nExp):
            ...
            perTime1 = duration / (t2 - t1)
            print(
                "HMM-based classification - segmentation \t {0:.1f} x realtime".format(
                    perTime1
                )
            )
        ...
        for i in range(nExp):
            ...
            segments = aS.silence_removal(
                x, Fs, 0.050, 0.050, smooth_window=1.0, Weight=0.3, plot=False
            )
            t2 = time.time()
            perTime1 = duration / (t2 - t1)
